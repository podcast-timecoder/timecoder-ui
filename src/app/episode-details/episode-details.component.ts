...
export class EpisodeDetailsComponent implements OnInit {
  ...
  diff = interval(1000).pipe(
    map(
      x =>
        (new Date().getTime() - +new Date(this.episode.startTime).getTime()),
      share()
    )
  );
  ...
  onSubmit() {
    this.episodeService
      .addTheme(this.episode.id, this.addForm.value)
      .subscribe(data => {
        this.getEpisodeDetails();
      });
    this.addForm.reset();
  ...
  updateTheme() {
    this.episodeService
      .updateTheme(this.editableTheme.id, this.updateForm.value)
      .subscribe(data => {
        this.getEpisodeDetails();
      });
  ...
  track(episode: Episode, theme: Theme) {
    this.episodeService.updateTimestamp(episode.id, theme).subscribe(data => {
      this.getEpisodeDetails();
    });
  }
  ...
  stopEpisode(episode: Episode) {
    this.episodeService.stopEpisode(episode).subscribe(data => {
      this.getEpisodeDetails();
    });
  }
  ...
  isFinished(episode: Episode): boolean {
    ...
    return (
      !episode.started &&
      episode.themeList.filter(val => val.passed === true).length > 0
    );
  }
  ...
  unlinkThemes(id: number, themeId: number) {
    this.episodeService.unlinkThemesToEpisode(id, themeId).subscribe(data => {
      this.getEpisodeDetails();
    });
  }
